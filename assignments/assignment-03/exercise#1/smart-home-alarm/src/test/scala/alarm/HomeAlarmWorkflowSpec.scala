...
import alarm.ArmingMode.PartialArm
import alarm.Zone.*
...
class HomeAlarmWorkflowSpec extends ScalaTestWithActorTestKit, AnyWordSpecLike, Eventually, Matchers:
  ...
  private val cfg = AlarmConfig(
    ...
    knownZones = Set(Perimeter, LivingArea),
  )
  ...
  "AlarmControlUnit" should:
    "not activate the siren if not armed" in:
      val (acu, sirenProbe) = freshAcu()
      acu ! SensorTriggered(UUID.randomUUID(), "motion", Perimeter)
      sirenProbe.expectNoMessage(400.millis)

    "ignore wrong PIN while disarmed" in:
      val (acu, sirenProbe) = freshAcu()
      acu ! PinEntered("0000")
      acu ! SensorTriggered(UUID.randomUUID(), "motion", Perimeter) // would alarm if armed
      sirenProbe.expectNoMessage(400.millis)
    ...
    "ignore PIN input during exit delay" in:
      ...
      acu ! SensorTriggered(UUID.randomUUID(), "door", Perimeter)
    ...
    "not re-trigger entry delay if a second sensor fires during entry delay" in:
      val (acu, sirenProbe) = armAcu()
      acu ! SensorTriggered(UUID.randomUUID(), "door", Perimeter)    // → entry delay starts
      Thread.sleep(50)
      acu ! SensorTriggered(UUID.randomUUID(), "motion", Perimeter)  // ignored — already counting
    ...
    "ignore sensor triggers while in alarm state" in:
      ...
      acu ! SensorTriggered(UUID.randomUUID(), "motion", Perimeter)
      sirenProbe.expectMessage(cfg.entryDelay + 400.millis, ActivateSiren)
      acu ! SensorTriggered(UUID.randomUUID(), "door", Perimeter)   // ignored in alarm
      sirenProbe.expectNoMessage(300.millis)             // no second ActivateSiren

    // NEW TESTS FOR ZONE-BASED ARMING

    "ignore sensor triggers from zones not in active set during PartialArm" in:
      val sirenProbe = createTestProbe[SirenCommand]()
      val acu = spawn(AlarmControlUnit(sirenProbe.ref, cfg.copy(knownZones = Set(Perimeter, LivingArea))))

      // Arm only Perimeter zone
      acu ! PinEntered("1234", PartialArm(Set(Perimeter)))
      Thread.sleep(cfg.exitDelay.toMillis + 100)

      // Trigger in armed zone — should start entry delay
      acu ! SensorTriggered(UUID.randomUUID(), "motion", Perimeter)
      sirenProbe.expectNoMessage(100.millis) // less than entryDelay

      // Now disarm (before entry delay expires)
      acu ! PinEntered("1234")

      // Trigger in non-armed zone — should be ignored even though we re-arm later
      acu ! SensorTriggered(UUID.randomUUID(), "motion", LivingArea)
      sirenProbe.expectNoMessage(500.millis)

    "activate siren when sensor fires in monitored zone with PartialArm" in:
      val sirenProbe = createTestProbe[SirenCommand]()
      val acu = spawn(AlarmControlUnit(sirenProbe.ref, cfg.copy(knownZones = Set(Perimeter, LivingArea))))

      acu ! PinEntered("1234", PartialArm(Set(LivingArea)))
      Thread.sleep(cfg.exitDelay.toMillis + 100)
